...
function renderMap() {
  ...
  const trace = {
    ...
    marker: {
      ...
      color: mapPoints.map((item) => item.article.id === state.selectedArticleId ? "#df6f73" : "#23613f"),
      opacity: 0.9,
...
function entityTextColor(type) {
  return type === "year" ? "#ffffff" : "#153024";
}
...
function renderEntityGraph(mode) {
  ...
  if (!data.allCount) {
    el.graphSvg.innerHTML = '<rect x="0" y="0" width="1400" height="900" fill="transparent"></rect><text x="700" y="450" text-anchor="middle" font-size="20" fill="#557063">Нет данных для отображения графа</text>';
    return;
  }
  ...
  parts.push(`<text x="${width / 2}" y="34" text-anchor="middle" font-size="18" font-weight="700" fill="#183528">${escapeHtml(title)}</text>`);
  if (data.hiddenCount) {
  ...
  data.edges.forEach((edge) => {
    ...
    const stroke = selected ? "#23613f" : "#b7cbc0";
  ...
  Object.values(positions).forEach(({ x, y, width: nodeWidth, node }) => {
    ...
    parts.push(`<text x="${x}" y="${y + 14}" text-anchor="middle" font-size="10" fill="${node.type === "year" ? "#eafff1" : "#60796d"}">${escapeHtml(node.type === "article" ? node.id : node.type)}</text>`);
    parts.push("</g>");
  });
...
function renderSimilarityGraph() {
  ...
  const parts = [`<rect x="0" y="0" width="${width}" height="${height}" fill="transparent"></rect>`];
  parts.push('<text x="700" y="46" text-anchor="middle" font-size="18" font-weight="700" fill="#183528">Связи статей</text>');
  ...
  filteredArticles.forEach((article) => {
    ...
    parts.push(`<rect x="${pos.x - 220}" y="${pos.y - 34}" width="440" height="68" rx="12" fill="#ffffff" stroke="#2d6a47" stroke-width="1.5"></rect>`);
    parts.push(`<text x="${pos.x}" y="${pos.y - 8}" text-anchor="middle" font-size="13" fill="#153024" font-weight="700">${escapeHtml(truncateLabel(article.title, 62))}</text>`);
    parts.push(`<text x="${pos.x}" y="${pos.y + 12}" text-anchor="middle" font-size="11" fill="#4e6b5d">${escapeHtml(String(article.year || "—"))} • ${escapeHtml(truncateLabel(article.journal || "Без журнала", 48))}</text>`);
    parts.push(`<text x="${pos.x}" y="${pos.y + 28}" text-anchor="middle" font-size="10" fill="#6a8277">ID: ${escapeHtml(article.id)}</text>`);
    parts.push("</g>");
  });
...
function populateEntityFilters() {
  const source = getCurrentSourceArticles();
  const j = state.filters.journal;
  const au = state.filters.author;
  const af = state.filters.affiliation;
  const forYear = source.filter((a) => {
    if (j && a.journal !== j) return false;
    if (au && !(a.authors || []).includes(au)) return false;
    if (af && !(a.affiliations || []).map((x) => x.place).includes(af)) return false;
    return true;
  });
